...
test.describe("Lägg till bok", () => {
    ...
    test("Lägger till EN bok och kollar sedan och den finns i katalogen", async ({ page }) => {
    ...
    test("Lägger till flera böcker och kollar sedan och de finns i katalogen", async ({ page }) => {
        const books = [
            { title: "Morgonstjärnan", author: "Karl Ove Knausgård" },
            { title: "Vargarna från evighetens skog", author: "Karl Ove Knausgård" },
    ...
    test("Lägger inte till med tomma fält och kan lägga till när båda är i fyllda", async ({ page }) => {
        const titleInput = page.getByTestId("add-input-title")
        const authInput = page.getByTestId("add-input-author")
